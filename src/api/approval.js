...
const settingApi = {
  ...
  processHeader: baseFrame + 'process-define/form/detail/definekey/',//获取审批表头
  processDetail: baseFrame + 'business/execution/',//获取审批详情
...
export function processHeader({definekey}) {
  return axios.get(settingApi.processHeader+definekey)
}
export function getProcessDetail({id}) {
  return axios.post(settingApi.processDetail+id+'/variables')
}
